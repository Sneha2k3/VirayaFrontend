"use client"
import { getAllMugs, getByCategory, getByMultipleFilters, getSubCategories } from '@/services/products';
...
import { FiArrowRight, FiDollarSign, FiFilter, FiPackage, FiShoppingCart, FiX } from 'react-icons/fi';
...
const Mug = () => {
  ...
  const { data: productsData, isLoading, error } = useQuery<ProductsData>({
    queryKey: ['mug-products', page, ITEMS_PER_PAGE, selectedSize, priceRange, searchQuery],
    queryFn: async () => {
      try {
        ...
        return await getAllMugs(page, ITEMS_PER_PAGE, obj);
      } catch (err) {
        console.log(err);
        throw err;
      }
  ...
  **/
  ...
  const filterOptions = {
    ...
    // subCategories: subCategoryData[0]?.subCategories?.map((item: any) => item?.name) //eslint-disable-line @typescript-eslint/no-explicit-any
        ...
        <div className="relative h-full max-w-7xl mx-auto px-4 flex items-center">
          <div className="max-w-3xl">
            ...
            <p className="text-lg md:text-xl text-gray-100 mb-8">
              Discover our unique collection of ceramic mugs crafted with care and ready to be personalized, perfect for everyday comfort or meaningful gifting.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-12">
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <div className={`md:w-1/4 bg-white rounded-xl shadow-md mt-20 p-6 h-fit
            ...
            {/* <AccordionItem key="subCategories" title="Sub Categories" classNames={{
                title: "!text-black"
              }}>
                <CheckboxGroup
          ...
          <div className="md:w-3/4">
            ...
            {/* Products Grid */}
            <div>
              <h1 className={`text-primary font-bold text-4xl mb-8 ${josefin.className}`}>Mug Collection</h1>
